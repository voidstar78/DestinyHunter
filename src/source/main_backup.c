...
{	
  ...
  // JACK the younger
	...
	initialize_challengeI(&challenges[0], 5,     43, 12,  4,  8UL,  60);	
	                                //                  
	                                //   num  N   x   y loiter
	initialize_challengeT(&challenges[0], 4,  0, 30, 10, 10);
	initialize_challengeT(&challenges[0], 4,  1, 25,  9, 8);
	initialize_challengeT(&challenges[0], 4,  2, 17, 12, 10);		
	...
	initialize_challengeI(&challenges[1], 5,     44, 18,  3, 20UL,  40);
	                                //                  
	                                //        num  N   x   y  loiter
	initialize_challengeT(&challenges[1], 4,  0, 32, 16, 20);
	initialize_challengeT(&challenges[1], 4,  1, 20, 20, 15);
	initialize_challengeT(&challenges[1], 4,  2, 14, 14, 10);		
}
...
void initialize_stage2_challenges()
{
  ...
  // JERRY the komodo
	...
	initialize_challengeT(&challenges[0], 4,  0, 22,  5, 10);
	initialize_challengeT(&challenges[0], 4,  1, 18, 10, 10);
	initialize_challengeT(&challenges[0], 4,  2, 15, 20, 10);		
	initialize_challengeT(&challenges[0], 4,  3, 37, 12, 15);
...
void initialize_stage3_challenges(unsigned char start, unsigned char count)
{
    ...
    switch (rand_mod(2))
		...
		//initialize_challengeI(&pvec_challenges[i], 5,     rand_mod(118)-40,  rand_mod(62)-20,  2,  1);	 
																		...
																		//                  rng
																		//       num   N   X   Y ofs loiter
		initialize_challengeT(&challenges[start], 4,  0, rand_mod(19)+9,  rand_mod(20)+2, rand_mod(10)+8);
		initialize_challengeT(&challenges[start], 4,  1, rand_mod(19)+9,  rand_mod(20)+2, rand_mod(10)+8);
		initialize_challengeT(&challenges[start], 4,  2, rand_mod(19)+9,  rand_mod(20)+2, rand_mod(10)+8);		
...
{	  
    ...
    switch (rand_mod(4))
		...
		initialize_challengeT(&challenges[start], 4,  1, rand_mod(38),  rand_mod(20)+2, rand_mod(6)+3);
		initialize_challengeT(&challenges[start], 4,  2, rand_mod(38),  rand_mod(20)+2, rand_mod(6)+3);		
...
void initialize_stage5_challenges()
{
  ...
  // JAMES the scorpion
	...
	initialize_challengeT(&challenges[0], 4,  1, 35, 12, rand_mod(10)+7);
	initialize_challengeT(&challenges[0], 4,  2, 18, 18, rand_mod(10)+6);		
	initialize_challengeT(&challenges[0], 4,  3, 25, 16, rand_mod(10)+5);
...
{	
  ...
  // JOSEPH the drake
	...
	initialize_challengeT(&challenges[0], 4,  2, 12,  6,  3);		
				...
				else
				{
					...
					{						
						...
						if (ptr_challenge->behavior == BEHAVIOR_LOITERING)  // REMINDER: all challenges start in GOING state...
						{
							...
							{								
								--(ptr_challenge->targets[ ptr_challenge->target_current ].loiter_current);
								
								if ((ptr_challenge->targets[ ptr_challenge->target_current ].loiter_current) == 0)
								{
									ptr_challenge->targets[ ptr_challenge->target_current ].loiter_current =
										ptr_challenge->targets[ ptr_challenge->target_current ].loiter_max;
					    ...
					    if (ptr_challenge->targetN > 0)  // this is a piece that can move - check decision to move
							{
								...
								{									
								  ...
								  // As used here, these are not really DELTA values.  They are absolute x/y screen coordinates, +1 area around the challenge icon
									...
									x_delta = (ptr_challenge->x - ptr_challenge->targets[ (ptr_challenge->target_current) ].target_x);
									y_delta = (ptr_challenge->y - ptr_challenge->targets[ (ptr_challenge->target_current) ].target_y);
...
move_due_to_loiter:
									...
									if (x_delta != 0)
									{
										...
										{		
										  ...
										  )
											{
												...
												ptr_challenge->direction_state = 0;
											}
											else if (temp_x < 38)
											{
												// move +1
												...
												{
												  ptr_challenge->direction_state = 1;
												}
											}
										}
									}							
									...
									)
									{
										...
										if (
										  (
											...
											//|| (READ_CHAR(temp_x, temp_y) == ICON_BOX)
									  )
										{
									...
									if (
									  (  // reached the current target
										...
										|| (ptr_challenge->stuck_count > 6)  // hasn't moved in awhile (got stuck on rocks, etc)
									)
									{
